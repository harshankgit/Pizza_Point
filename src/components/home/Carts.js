  ...
  switch (action.type) {
    ...
    case "ADD":
      ...
      if (existingIndex >= 0) {
        ...
        updatedState[existingIndex].quantity += action.payload.quantity;
        return updatedState;
      }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {products.map((product) => (
          ...
          >
            ...
            <select
              onChange={(e) =>
                addToCart(product, 1, e.target.value || product.options[0])
              }
              className="w-full border rounded px-2 py-1"
            >
              {product.options.map((option, index) => (
            ...
            <p className="text-lg font-bold text-gray-850 mt-2">
              Price: ₹{product.price[product.options[0]]}
            </p>
            <button
              onClick={() => addToCart(product, 1, product.options[0])}
